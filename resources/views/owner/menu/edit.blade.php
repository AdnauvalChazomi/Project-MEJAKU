@extends('layouts.app')
@section('title', 'Edit Menu | MejaKu')

@section('content')
<div x-data="{ showSuccessPopup: false }" class="min-h-screen bg-gray-50 px-5 md:px-10 py-8 space-y-8">
    ...
    <header class="flex items-center gap-3">
        ...
        <h1 class="text-xl md:text-2xl font-semibold text-gray-900">Edit Menu</h1>
    </header>
    ...
    <form action="{{ route('menu.update', ['id' => $menu->id]) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="space-y-2">
            ...
            @if ($menu->foto)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $menu->foto) }}" alt="Foto menu" class="w-32 h-32 object-cover rounded-lg shadow-sm">
                </div>
            @endif
        ...
        <div class="space-y-2">
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Tulis deskripsi singkat..."
                class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
            @error('deskripsi')
                <p class="text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-3 pt-4">
            <a href="{{ route('menu.index', ['id' => $menu->owner_id]) }}"
