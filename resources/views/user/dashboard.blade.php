        ...
        <section class="px-6 lg:px-20 py-10 max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @forelse($rekomendasi as $resto)
                    <a href="{{ route('user.restoran.show', $resto->id) }}" class="block">
                        <div class="bg-white rounded-xl shadow-md overflow-hidden hover:scale-[1.03] transition transform">
                            <div class="w-full aspect-square overflow-hidden">
                                <img src="{{ $resto->foto_restoran ? asset('storage/' . $resto->foto_restoran) : 'https://via.placeholder.com/400x400' }}"
                                    alt="{{ $resto->nama_restoran }}" class="w-full h-full object-cover">
                            </div>
                            <div class="p-3">
                                <h3 class="font-semibold text-gray-900 text-sm md:text-base truncate">
                                    {{ $resto->nama_restoran }}
                                </h3>
                                <p class="text-gray-500 text-xs md:text-sm truncate">{{ $resto->alamat_restoran }}</p>
                                <div class="flex items-center mt-1 text-yellow-500 text-sm">
                                    ...
                                    <span
                                        class="ml-1 text-gray-700">{{ number_format($resto->reviews->avg('rating') ?? 0, 1) }}</span>
                                </div>
                            </div>
                        </div>
                    </a>
