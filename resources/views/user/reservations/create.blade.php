...
@section('navbar')
    <header class="flex items-center justify-between px-4 py-3 border-b sticky top-0 bg-white z-30">
        ...
        <h1 class="text-lg font-bold text-[#9D3935] tracking-tight">Reservasi</h1>
        <div class="w-6"></div>
    </header>
@endsection
        ...
        <div class="max-w-lg mx-auto">
            ...
            <form action="{{ route('user.reservations.store') }}" method="POST"
                ...
                <button type="button" id="confirmReservationBtn"
                    class="w-full py-3 bg-[#9D3935] text-white font-semibold rounded-lg shadow-md hover:bg-red-700 transition">
                    Lanjut Pilih Menu
                </button>
            </form>
                    ...
                    @forelse ($restoran->menuUnggulan as $unggulan)
                        <div x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false"
                            ...
                            <div class="p-2 text-center transition-all duration-300">
                                <h4 class="text-sm font-semibold text-gray-800 group-hover:text-[#9D3935]">
                                    {{ $unggulan->menu->nama }}
                                </h4>
